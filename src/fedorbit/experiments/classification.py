...
def _simplification_rule_states(
    ...
    return (
        _rule_payload(
            SimplificationRuleName.RECTANGULARIZATION_IS_SUFFICIENT, rectangular, rectangular_reason
        ),
        _rule_payload(SimplificationRuleName.GENERIC_QAP_DOMINATES, qap_state, qap_reason),
        _rule_payload(
            SimplificationRuleName.SPARSE_SUPPORT_IS_OPERATIONALLY_IRRELEVANT,
            sparse_state,
            sparse_reason,
        ),
        _rule_payload(
            SimplificationRuleName.POINT_MATCHING_IS_SUFFICIENT, point_state, point_reason
        ),
        _rule_payload(
            SimplificationRuleName.STRICT_INTERFACE_REMOVES_GAIN, interface_state, interface_reason
        ),
        _rule_payload(
            SimplificationRuleName.SOURCE_RESPONSE_IS_TOO_UNSTABLE, source_state, source_reason
        ),
    )
