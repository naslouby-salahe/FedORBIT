...
def execute_statistical_synthesis(
    ...
    for pair in external_source_pairs:
        ...
        if len(shared_seeds) < statistics_config.minimum_valid_paired_seeds:
            unpaired_nonzero_count: Index = 0
            unpaired_bootstrap_seed: RandomSeed = 0
            external_source_contrasts[pair] = (
                ...
                None,
                input_ids,
                unpaired_nonzero_count,
                unpaired_bootstrap_seed,
            )
            continue
        ...
        bootstrap_seed = statistical_bootstrap_seed(
            ...
            DirectedPairName(pair),
            MetricId.MACRO_CROSS_ENTROPY,
            BootstrapPurpose.EXTERNAL_SOURCE_VS_LOCAL_SIR_GAIN,
        )
    ...
    for pair in point_correspondence_pairs:
        ...
        bootstrap_seed = statistical_bootstrap_seed(
            ...
            DirectedPairName(pair),
            MetricId.MACRO_CROSS_ENTROPY,
            BootstrapPurpose.POINT_CORRESPONDENCE_SAFETY_DIFFERENCE,
        )
...
def _execute_ablation_and_sparsity_and_confirmation_statistical_synthesis(
    ...
    for pair in sparsity_pairs:
        ...
        for condition_a, condition_b in condition_pairs:
            ...
            if len(shared_seeds) < statistics_config.minimum_valid_paired_seeds:
                sparsity_unpaired_nonzero_count: Index = 0
                sparsity_unpaired_bootstrap_seed: RandomSeed = 0
                sparsity_contrasts[contrast_key] = (
                    ...
                    None,
                    input_ids,
                    sparsity_unpaired_nonzero_count,
            ...
            bootstrap_seed = statistical_bootstrap_seed(
                ...
                BootstrapPurpose.SPARSITY_SENSITIVITY_GAIN_DIFFERENCE,
            )
    ...
    for pair in confirmation_pairs:
        ...
        if len(shared_seeds) < statistics_config.minimum_valid_paired_seeds:
            confirmation_unpaired_nonzero_count: Index = 0
            confirmation_unpaired_bootstrap_seed: RandomSeed = 0
            confirmation_contrasts[pair] = (
                ...
                input_ids,
                None,
                confirmation_unpaired_nonzero_count,
        ...
        bootstrap_seed = statistical_bootstrap_seed(
            ...
            BootstrapPurpose.CONFIRMATION_SAFETY_HARM_RATE_DIFFERENCE,
        )
