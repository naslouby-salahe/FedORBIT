...
class RetryDecision:
    retry: bool
    remaining_attempts: RetryCount
    terminal_state: TerminalState | None = None


class RetryPolicy:
    def __init__(self, retries_after_initial: RetryCount) -> None:
    ...
    def decide(self, attempt_index: Index, classification: FailureClassification) -> RetryDecision:
        if not classification.retryable:
            no_remaining_attempts: RetryCount = 0
            return RetryDecision(
                retry=False,
                remaining_attempts=no_remaining_attempts,
        ...
        remaining_attempts: RetryCount = remaining
        return RetryDecision(retry=True, remaining_attempts=remaining_attempts)
...
class ExecutionOutcome:
    ...
    completed_support_records: tuple[SupportRecordIdentifier, ...] = ()
    cut_master_counters: CutMasterCounters = ()
...
def validation_failure_outcome(reason: FailureReason, invalid: bool) -> ExecutionOutcome:
    return ExecutionOutcome(
