...
def _build_assignment_structure(
    ...
    for block_index in range(len(blocks.padded_size_tuple)):
        ...
        for target in targets:
            variables = [
                model.addVar(vtype="B", name=f"{prefix}_p_{source}_{target}") for source in sources # TODO: should be enum
            ]
...
def _add_mccormick_products(
    ...
    for key, coefficient in sorted(coefficients.items()):
        ...
        y: Expr = model.addVar(vtype="C", lb=0.0, ub=1.0, name=f"y_{prefix}_{key}") # TODO: should be enum
...
def fixed_action_worst_correspondence_qap(
    ...
    model = Model("qap_fixed_action") # TODO: should be enum
    ...
    model.setObjective(quicksum(objective_terms) if objective_terms else 0.0, "minimize") # TODO: should be enum
    model.optimize()
...
def point_correspondence_commitment(
    ...
    model = Model("qap_point_correspondence") # TODO: should be enum
    _configure_model(model, deadline)
