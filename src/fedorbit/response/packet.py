...
class SourcePacket:
    ...
    def from_serialized(cls, payload: SerializedPacket) -> SourcePacket:
        ...
        if any(
            array.dtype != "float64" # TODO: should be enum
            or array.order != "C" # TODO: should be enum
    ...
    def _response_matrix(self, entries: ResponseBounds) -> np.ndarray:
        ...
        return np.asarray(entries, dtype=np.float64).reshape((node_count, node_count), order="C") # TODO: should be enum
...
def construct_source_packet(
    ...
    ordering = anonymous_node_order(
        ...
        AnonymityCoordinate(
            (
                AnonymityCoordinateEntry(
                    "source_checkpoint_sha256", # TODO: should be enum
                    context.source_checkpoint_sha256,
                ),
                AnonymityCoordinateEntry(
                    "response_configuration_sha256", # TODO: should be enum
...
def _float64_array(values: tuple[Index | Estimate, ...]) -> Float64ArrayPayload:
    array = np.asarray(values, dtype=np.float64, order="C") # TODO: should be enum
    return Float64ArrayPayload(
        dtype=PacketArrayDtype.FLOAT64,
        order=PacketArrayOrder.C,
        shape=tuple(int(size) for size in array.shape),
        data=tuple(float(value) for value in array.ravel(order="C")), # TODO: should be enum
    )
