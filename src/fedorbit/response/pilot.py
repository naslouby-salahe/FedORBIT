...
def _eligibility_reasons(
    ...
    if not all_finite:
        reasons.append(IneligibilityReason("non-finite shadow state or loss")) # TODO: should be enum
    useful_entries = tuple(entry for entry in entries if entry.useful)
    if not useful_entries:
        reasons.append(IneligibilityReason("no useful entries")) # TODO: should be enum
        return reasons
    if (
        ...
        reasons.append(IneligibilityReason("median derivative discrepancy above ceiling")) # TODO: should be enum
    if (
        statistics.median(tuple(entry.sign_agreement for entry in useful_entries))
        < pilot.sign_agreement_minimum
    ):
        reasons.append(IneligibilityReason("median sign agreement below minimum")) # TODO: should be enum
    if len(useful_columns) < pilot.minimum_useful_intervention_columns:
        reasons.append(IneligibilityReason("too few useful intervention columns")) # TODO: should be enum
    return reasons
