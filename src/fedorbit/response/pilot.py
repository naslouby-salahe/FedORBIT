...
def run_pooled_source_response_pilot(
    ...
    if len(pilot_checkpoints) != registered_seed_count:
        raise ResponsePilotError("source-response pilot requires one checkpoint per pilot seed")
    if len({checkpoint.seed for checkpoint in pilot_checkpoints}) != registered_seed_count:
...
def _eligibility_reasons(
    ...
    if not all_finite:
        reasons.append(IneligibilityReason.NON_FINITE_SHADOW_STATE)
    useful_entries = tuple(entry for entry in entries if entry.useful)
    if not useful_entries:
        reasons.append(IneligibilityReason.NO_USEFUL_ENTRIES)
        return reasons
    if (
        ...
        reasons.append(IneligibilityReason.DERIVATIVE_DISCREPANCY_ABOVE_CEILING)
    if (
        statistics.median(tuple(entry.sign_agreement for entry in useful_entries))
        < pilot.sign_agreement_minimum
    ):
        reasons.append(IneligibilityReason.SIGN_AGREEMENT_BELOW_MINIMUM)
    if len(useful_columns) < pilot.minimum_useful_intervention_columns:
        reasons.append(IneligibilityReason.TOO_FEW_USEFUL_INTERVENTION_COLUMNS)
    return reasons
