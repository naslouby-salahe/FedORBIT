...
AUTHORITATIVE_ENUM_MODULE = "fedorbit.domain.enums"
BOUNDARY_CONSUMER_PACKAGES = {"config", "runtime", "artifacts", "execution", "cli"}


def test_domain_enums_are_consumed_by_boundary_packages() -> None:
    from tests.architecture.scan import SRC_ROOT

    enum_path = SRC_ROOT / "domain" / "enums.py"
    tree = parse_module(enum_path)
    enum_names = {klass.name for klass in public_classes(tree)}
    assert enum_names
    ...
    for path in iter_source_files():
        ...
        if "from fedorbit.domain.enums import" in text:
            consumers.add(str(path))
    assert consumers, "no production module imports the domain enum catalogue"


def test_each_domain_enum_is_referenced_by_production_code() -> None:
    from tests.architecture.scan import SRC_ROOT
    ...
    for enum_name in enum_names:
        assert enum_name in production_text, (
            f"domain enum {enum_name} is never referenced by production code"
        )
