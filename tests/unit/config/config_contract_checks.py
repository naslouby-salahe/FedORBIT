...
_experiment_local_method_names = {candidate.value for candidate in ExperimentLocalMethod}
...
def _validate_split_intervals(config: FedorbitConfig) -> None:
    ...
    for name, (lower, upper) in named.items():
        ...
        _require(
            lower < upper,
            ValidationReason(f"split interval {name} must have positive width"),
        )
        _require(
            upper <= 1.0 + numerical_tolerance,
            ValidationReason(f"split interval {name} must end at or before 1.0"),
        )
        previous_upper = upper
    _require(
        abs(previous_upper - 1.0) <= numerical_tolerance,
        ValidationReason("split intervals must cover the full [0, 1] range"),
    )
...
def _validate_seeds(config: FedorbitConfig) -> None:
    ...
    _require(
        len(randomness.pilot_seeds) == 3,
        ValidationReason("pilot seed list must contain exactly 3 seeds"),
    )
    _require(
        len(randomness.confirmatory_seeds) == 10,
        ValidationReason("confirmatory seed list must contain exactly 10 seeds"),
    )
    _require(
        len(set(randomness.pilot_seeds)) == 3,
        ValidationReason("pilot seeds must be distinct"),
    )
    _require(
        len(set(randomness.confirmatory_seeds)) == 10,
        ValidationReason("confirmatory seeds must be distinct"),
    )
    _require(
        all(seed > 0 for seed in randomness.pilot_seeds),
        ValidationReason("pilot seeds must be positive"),
    )
    _require(
        all(seed > 0 for seed in randomness.confirmatory_seeds),
        ValidationReason("confirmatory seeds must be positive"),
    )
    _require(randomness.statistical_seed > 0, ValidationReason("statistical seed must be positive"))
...
def _validate_action(config: FedorbitConfig) -> None:
    ...
    _require(
        ...
        ValidationReason("principal sparse support must be at least 1"),
    )
    _require(
        action.principal_sparse_support <= 3,
        ValidationReason("principal sparse support must be at most 3"),
    )
    _require(
        all(support in (1, 2, 3) for support in action.sparse_support_sensitivity),
        ValidationReason("sparse support sensitivity values must belong to {1, 2, 3}"),
    )
    _require(
    ...
    _require(
        action.total_curriculum_budget > 0.0,
        ValidationReason("total curriculum budget must be positive"),
    )
    _require(action.coordinate_cap > 0.0, ValidationReason("coordinate cap must be positive"))
    _require(
        action.coordinate_cap <= action.total_curriculum_budget,
        ValidationReason("coordinate cap must not exceed the total curriculum budget"),
    )
    _require(
        action.linear_cost_per_actionable_node >= 0.0,
        ValidationReason("linear cost per actionable node must be nonnegative"),
    )
    _require(
        action.maximum_source_proposals_per_target >= 1,
        ValidationReason("maximum source proposals per target must be at least 1"),
    )
...
def _validate_datasets(config: FedorbitConfig) -> None:
    ...
    for pair in (*datasets.primary_directed_pairs, *datasets.secondary_directed_pairs):
        _require(
            pair.source in registered, ValidationReason(f"unknown source client in pair {pair}")
        )
        _require(
            pair.target in registered, ValidationReason(f"unknown target client in pair {pair}")
        )
        _require(
            pair.source != pair.target,
            ValidationReason(f"self-pair is not a directed pair: {pair}"),
        )
    _require(
        len(datasets.primary_directed_pairs) == 6,
        ValidationReason("exactly six primary directed ToN-IoT pairs are registered"),
    )
    _require(
        len(datasets.secondary_directed_pairs) == 0,
        ValidationReason("no secondary pairs are registered while Edge-IIoTset is external-only"),
    )
    ...
    _require(
        ...
        ValidationReason("primary directed pairs must involve exactly the three primary clients"),
    )
    _require(
        primary_targets
        == {
        ...
        ValidationReason("primary directed pairs must target exactly the three primary clients"),
    )
    _require(
    ...
    _require(
        ...
        ValidationReason("the ToN-IoT host clients must have role primary"),
    )
    _require(
        roles[DatasetId.TON_IOT_NETWORK] == ClientRole.PRIMARY
        and roles[DatasetId.EDGE_IIOTSET_NETWORK] == ClientRole.EXTERNAL,
        ValidationReason("the ToN-IoT network client must be primary and Edge-IIoTset external"),
    )


def _validate_training(config: FedorbitConfig) -> None:
    training = config.scientific.training
    _require(training.maximum_epochs >= 1, ValidationReason("maximum epochs must be at least 1"))
    _require(training.batch_size >= 1, ValidationReason("batch size must be at least 1"))
    _require(training.dataloader_workers == 0, ValidationReason("dataloader workers must be 0"))
    _require(
        training.early_stopping.patience_completed_epochs >= 0,
        ValidationReason("early-stopping patience must be nonnegative"),
    )
    ...
    _require(
        ...
        ValidationReason("pilot learning rates must be positive"),
    )
...
def _validate_response(config: FedorbitConfig) -> None:
    ...
    _require(
        ...
        ValidationReason("source-response pilot grid must contain 3 magnitudes and 3 horizons"),
    )
    _require(
        pilot.paired_schedules_per_candidate >= 1,
        ValidationReason("paired schedules per candidate must be at least 1"),
    )
    final = config.scientific.source_response_final
    _require(
        final.paired_replicates_per_intervention >= 1,
        ValidationReason("paired replicates per intervention must be at least 1"),
    )
    _require(
        0.0 < final.simultaneous_confidence_level < 1.0,
        ValidationReason("simultaneous confidence level must lie in (0, 1)"),
    )
    ...
    _require(
        ...
        ValidationReason("target-local response diagnostic counts must be positive"),
    )
...
def _validate_confirmation(config: FedorbitConfig) -> None:
    ...
    _require(
        ...
        ValidationReason("confirmation counts must be positive"),
    )
    _require(
        0.0 < confirmation.one_sided_confidence_level < 1.0,
        ValidationReason("one-sided confidence level must lie in (0, 1)"),
    )
...
def _validate_statistics(config: FedorbitConfig) -> None:
    ...
    _require(
        ...
        ValidationReason("nominal confidence level must lie in (0, 1)"),
    )
    _require(
        statistics.ci_bootstrap_repetitions >= 1 and statistics.minimum_valid_paired_seeds >= 1,
        ValidationReason("statistical counts must be positive"),
    )
    _require(
        0.0 < statistics.tost_alpha_per_one_sided_test < 1.0,
        ValidationReason("TOST alpha must lie in (0, 1)"),
    )
    _require(
        statistics.spearman_minimum_valid_points >= 2,
        ValidationReason("Spearman minimum valid points must be at least 2"),
    )
    _require(
        statistics.exact_sign_flip_max_nonzero_differences_for_enumeration >= 1,
        ValidationReason("exact sign-flip enumeration limit must be at least 1"),
    )
...
def _validate_evaluation_criteria(config: FedorbitConfig) -> None:
    ...
    _require(
        ...
        <= primary_pair_count,
        ValidationReason("external-source required pairs must not exceed the primary pair count"),
    )
    _require(
    ...
    _require(
        ...
        ValidationReason("sparse operational relevance unit fraction must lie in (0, 1]"),
    )
    _require(
        criteria.sparse_operational_relevance.compared_sparse_support in (1, 2, 3),
        ValidationReason("compared sparse support must belong to {1, 2, 3}"),
    )
...
def _validate_simplification_rules(config: FedorbitConfig) -> None:
    ...
    _require(
        ...
        ValidationReason("rectangularization trigger fraction must lie in (0, 1]"),
    )
    ...
    _require(
        ...
        ValidationReason("source-response instability trigger fraction must lie in (0, 1]"),
    )
    _require(
        all(
            support in (1, 2, 3)
        ...
        ValidationReason("sparse supports in the irrelevance rule must belong to {1, 2, 3}"),
    )
    _require(
        rules.generic_qap_dominates.intended_sparse_support_maximum in (1, 2, 3),
        ValidationReason("generic QAP dominates intended support maximum must belong to {1, 2, 3}"),
    )
...
def _validate_generators(config: FedorbitConfig) -> None:
    ...
    _require(
        ...
        ValidationReason("exact-separator theorem generator supports must belong to {1, 2, 3}"),
    )
    _require(
        all(support in (1, 2, 3) for support in generators.coupling_structure.supports),
        ValidationReason("coupling-structure generator supports must belong to {1, 2, 3}"),
    )
    _require(
        ...
        ValidationReason("generator rejection-sampling attempt limits must be positive"),
    )
...
def _validate_solvers(config: FedorbitConfig) -> None:
    ...
    _require(
        ...
        ValidationReason("maximum concurrently executed supports must be at least 1"),
    )
    _require(
        exact_sparse.lp_threads_per_solve == 1,
        ValidationReason("exact-sparse LP threads per solve must be 1"),
    )
    _require(
        exact_sparse.maximum_cuts_per_support >= 1,
        ValidationReason("maximum cuts per support must be at least 1"),
    )
    _require(
        config.solvers.dense_ccp.deterministic_starts >= 1,
        ValidationReason("dense-CCP deterministic starts must be at least 1"),
    )
    _require(
        config.solvers.dense_ccp.outer_action_cuts >= 1,
        ValidationReason("dense-CCP outer action cuts must be at least 1"),
    )
...
def _validate_experiments(config: FedorbitConfig) -> None:
    ...
    _require(
        ...
        ValidationReason("exact-sparse benchmark K range must be valid"),
    )
    _require(
        all(support in (1, 2, 3) for support in experiments.exact_sparse_solver_benchmark.supports),
        ValidationReason("exact-sparse benchmark supports must belong to {1, 2, 3}"),
    )
    _require(
        all(
            support in (1, 2, 3)
        ...
        ValidationReason("scalability exact/QAP supports must belong to {1, 2, 3}"),
    )
    _require(
        all(
            support in (1, 2, 3)
        ...
        ValidationReason("weak-signal support budgets must belong to {1, 2, 3}"),
    )
    for experiment in (
        ...
        for method in experiment.methods:
            _require(
                ...
                ValidationReason(f"unregistered method name in experiment configuration: {method}"),
            )
    ...
    for target in experiments.multi_source_selection_validation.targets:
        _require(
            target in registered,
            ValidationReason(f"unknown multi-source validation target: {target}"),
        )
    _require(
        experiments.map_availability_applicability_audit.independent_researchers >= 1,
        ValidationReason("map-availability audit requires at least one independent researcher"),
    )
    _require(
        experiments.map_availability_applicability_audit.minutes_per_researcher_per_pair >= 1,
        ValidationReason("map-availability audit session duration must be positive"),
    )
    ...
    _require(
        ...
        ValidationReason("exact-sparse solver benchmark must include FedORBIT Exact-Sparse Solver"),
    )
    _require(
        TransferMethod.GENERIC_EXACT_QAP.value in benchmark_methods,
        ValidationReason("exact-sparse solver benchmark must include Generic Exact QAP"),
    )
    _require(
        TransferMethod.FEDORBIT_DENSE_CCP_FALLBACK.value in benchmark_methods,
        ValidationReason("exact-sparse solver benchmark must include FedORBIT Dense-CCP Fallback"),
    )
    ...
    _require(
        ...
        ValidationReason("primary strict transfer must include FedORBIT Exact-Sparse Solver"),
    )
    _require(
        TransferMethod.LOCAL_ONLY.value in transfer_methods,
        ValidationReason("primary strict transfer must include Local-Only"),
    )
...
def _validate_runtime(config: FedorbitConfig) -> None:
    ...
    _require(
        runtime.solver_cpu_worker_ceiling >= 1,
        ValidationReason("solver CPU worker ceiling must be at least 1"),
    )
    _require(
        runtime.deterministic_kernel_warmups >= 0
        and runtime.deterministic_kernel_timed_repetitions >= 1,
        ValidationReason("deterministic kernel timing counts must be valid"),
    )
    layout = runtime.artifact_layout
    _require(layout.execution_root == "outputs", ValidationReason("execution root must be outputs"))
    _require(
        layout.manuscript_root == "results", ValidationReason("manuscript root must be results")
    )
    _require(
        layout.preprocessing_subdirectories
        == ("inventories", "validation", "prepared", "splits", "features", "metadata"),
        ValidationReason("preprocessing subdirectories must be exactly the registered set"),
    )
    _require(
        layout.reusable_artifact_subdirectories
        == ("models", "scores", "fitted", "baselines", "derived"),
        ValidationReason("reusable artifact subdirectories must be exactly the registered set"),
    )
    _require(
        layout.cache_subdirectories
        == ("preprocessing", "models", "evaluation", "analysis", "staging"),
        ValidationReason("cache subdirectories must be exactly the registered set"),
    )
...
def _validate_reporting(config: FedorbitConfig) -> None:
    ...
    _require(
        ...
        ValidationReason("reporting decimal precisions must be nonnegative"),
    )
    _require(
        0.0 < precision.p_value_less_than_threshold < 1.0,
        ValidationReason("p-value display threshold must lie in (0, 1)"),
    )
