...
def test_split_intervals_exact_from_config(fedorbit_config: FedorbitConfig) -> None:
    assert interval_edges(fedorbit_config) == (
...
def test_split_boundaries_use_half_open_intervals_except_test_end(
    fedorbit_config: FedorbitConfig,
) -> None:
    assert split_for_duplicate_group(fedorbit_config, 0.55) == Split.META
    assert split_for_duplicate_group(fedorbit_config, 0.70) == Split.VALID
    assert split_for_duplicate_group(fedorbit_config, 0.80) == Split.CONFIRM
    assert split_for_duplicate_group(fedorbit_config, 0.90) == Split.TEST
    assert split_for_duplicate_group(fedorbit_config, 1.00) == Split.TEST
...
def test_chronological_assignment_uses_timestamp_then_hash_and_indivisible_groups(
    fedorbit_config: FedorbitConfig,
) -> None:
