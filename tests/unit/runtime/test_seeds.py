...
def test_derivation_matches_spec_formula() -> None:
    for base_seed in (101, 1103, 300, 5531):
        for namespace in RngNamespace:
            actual = derive_seed32(base_seed, namespace, FIXED_COORDINATES)
...
def test_derivation_is_stable_across_calls() -> None:
    first = derive_seed32(1103, RngNamespace.SPLIT, FIXED_COORDINATES)
    second = derive_seed32(1103, RngNamespace.SPLIT, FIXED_COORDINATES)
    assert first == second


def test_distinct_namespaces_yield_distinct_seeds() -> None:
    seeds = {derive_seed32(1103, namespace, FIXED_COORDINATES) for namespace in RngNamespace}
    assert len(seeds) == len(RngNamespace)


def test_distinct_base_seeds_yield_distinct_seeds() -> None:
    seeds = {
        derive_seed32(base, RngNamespace.SPLIT, FIXED_COORDINATES) for base in (101, 202, 303, 1103)
    }
    assert len(seeds) == 4


def test_distinct_coordinates_yield_distinct_seeds() -> None:
    first = derive_seed32(1103, RngNamespace.SPLIT, {"pair": "a"})
    second = derive_seed32(1103, RngNamespace.SPLIT, {"pair": "b"})
    assert first != second
...
def test_seed_plan_covers_every_namespace() -> None:
    plan = seed_plan(1103, FIXED_COORDINATES)
    assert len(plan.streams) == len(RngNamespace)
    for namespace in RngNamespace:
        assert plan.seed_for(namespace) == derive_seed32(1103, namespace, FIXED_COORDINATES)


def test_seed_plan_rejects_unknown_namespace() -> None:
    partial = SeedPlan(base_seed=1103, coordinates_json="{}", streams=((RngNamespace.SPLIT, 1),))
...
def test_scoped_numpy_generator_replays_identical_choices() -> None:
    first = numpy_generator(42).integers(0, 2**31, size=8)
    second = numpy_generator(42).integers(0, 2**31, size=8)
    assert list(first) == list(second)
    different = numpy_generator(43).integers(0, 2**31, size=8)
    assert list(first) != list(different)


def test_scoped_torch_generator_replays_identical_choices() -> None:
    first = torch.rand(8, generator=torch_generator(42))
    second = torch.rand(8, generator=torch_generator(42))
    assert torch.equal(first, second)
    different = torch.rand(8, generator=torch_generator(43))
    assert not torch.equal(first, different)


def test_statistical_bootstrap_streams_are_contrast_scoped() -> None:
    first = statistical_bootstrap_stream(300, {"contrast": "A", "family": "utility"})
    second = statistical_bootstrap_stream(300, {"contrast": "A", "family": "utility"})
    assert list(first.integers(0, 2**31, size=8)) == list(second.integers(0, 2**31, size=8))
    other = statistical_bootstrap_stream(300, {"contrast": "B", "family": "utility"})
...
def test_statistical_seed_is_used_only_for_bootstrap_streams() -> None:
    plan = seed_plan(300, FIXED_COORDINATES)
