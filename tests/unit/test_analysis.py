...
def test_holm_step_down_monotone_with_lexicographic_tie_break() -> None:
    ...
    c_value = adjusted.value_of("c")
    a_value = adjusted.value_of("a")
    b_value = adjusted.value_of("b")
    assert c_value == pytest.approx(min(1.0, 0.01 * 3))
    assert a_value == pytest.approx(min(1.0, 0.03 * 2))
    assert a_value is not None
    assert b_value == pytest.approx(max(a_value, min(1.0, 0.03 * 1)))
    values = tuple(entry.p_value for entry in adjusted.entries)
